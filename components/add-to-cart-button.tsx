...
export default function AddToCartButton({ product }: AddToCartButtonProps) {
  ...
  // VARIANT DIHAPUS: Menggunakan stok dan harga murni dari produk utama
  ...
  async function handleAddToCart() {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        // VARIANT DIHAPUS: Query mengecek item keranjang murni berdasarkan product_id saja
        const { data: existing } = await supabase
          .from("cart_items")
          .select("id, quantity")
          .eq("cart_id", cartId)
          .eq("product_id", product.id)
          .maybeSingle(); // Menggunakan maybeSingle agar tidak crash jika belum ada data
        ...
        if (existing) {
          ...
          // VARIANT DIHAPUS: Insert data murni tanpa menyertakan kolom variant_id
